...
#          `ms_separator`. Default: '\N{TRIANGULAR BULLET}'.
...
#          precedence over `ms_separator_replace`. Default: '-->'.
...
def gen_menu(text, sep, rep, space=True):
  ...
  Returns:
    list[nodes.Node] containing the rendered menuselection with custom separator.
  """
  if space:
    sep = ' %s ' % sep
...
class CmdMenu(SphinxRole):
  ...
  def run(self):
    sep = config.get_sep(
        self.inliner.document.settings.env.config.ms_cmdmenu_separator,
        self.inliner.document.settings.env.config.ms_separator)

    rep = config.get_rep(
        self.inliner.document.settings.env.config.ms_cmdmenu_separator_replace,
        self.inliner.document.settings.env.config.ms_separator_replace)

    return [gen_menu(self.text, sep, rep)], []


def setup(app):
  app.add_config_value('ms_cmdmenu_separator', config.DEFAULT_SEPARATOR, '')
  app.add_config_value('ms_cmdmenu_separator_replace', config.DEFAULT_REPLACE, '')
  app.add_config_value('ms_cmdmenu_replace_use_space', True, '')
